...
echo $MSG
export DEPLOY_DIR=$(zenity  --title "$MSG" --directory --file-selection --width=$W --height=150)
...
echo $MSG
MYSQL_CLIENT_JAR=$(zenity  --title "$MSG" --file-selection --width=$W --height=150)
...
then
	...
	then
		...
		echo $MSG
		SITE_SPECIFIC_POLICIES_FILE=$(zenity  --title "$MSG" --file-selection --width=$W --height=150)
